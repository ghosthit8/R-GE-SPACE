...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      // Compute current checkpoint
      let k = c.paused ? c.last_checkpoint : calcCheckpoint(c.cycle_start, c.period_sec);

      // Catch up if needed (server-driven, idempotent)
      if (!c.paused && k > c.last_checkpoint) {
        for (let x = c.last_checkpoint + 1; x <= k; x++) {
          await decideRoundOnce(c.cycle_start, x);
        }
        await supabase.from("cycles_v2")
          .update({ last_checkpoint: k, updated_at: new Date().toISOString() })
          .eq("id", c.id);
      }

      // --- auto-rollover after final (keeps looping forever) ---
      const periodMs = c.period_sec * 1000;
      const endMs = Date.parse(c.cycle_start) + periodMs;
      if (!c.paused && k >= CHECKPOINTS && Date.now() >= endMs) {
        const nextStart = new Date(endMs).toISOString(); // roll to the next window
        ...
        // reflect new values in response without extra round-trip
        c.cycle_start = nextStart;
        c.last_checkpoint = 0;
        k = 0;
      }

      // Compute response times (based on possibly updated cycle_start/last_checkpoint)
