  ...
  try {
    ...
    if (!open || open.length === 0) {
      if (curRound < FINAL_ROUND) {
        ...
        await s.from("matches").update({ votes_a:0, votes_b:0, decided:false }).eq("round", nextRound);
        await s.from("matches").update({ votes_a:0, votes_b:0 }).eq("round", curRound);
        await s.from("tournament_meta").upsert({ id:1, current_round: nextRound, duration_seconds: duration });
      } else {
    ...
    const endAt = await seedTimer(s, nextRound, duration);
